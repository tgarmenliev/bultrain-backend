 ...
 */
exports.getScheduleCurrentDate = async (req, res) => {
    ...
    try {
        ...
        // 2. Call the async scheduleController logic for today
        const result = await scheduleController.generateScheduleData(language, from, to, currentDateStr);

        if (result.error) {
            return res.status(result.status).json({ error: result.error });
        }

        // 3. Strip internal tracking fields (departMins, arriveMins)
